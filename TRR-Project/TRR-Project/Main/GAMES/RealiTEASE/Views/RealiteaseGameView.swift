...
struct RealiteaseGameView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .onAppear {
                ...
                manager.fetchTodayAnswer(for: .specific(gameDate))
            }
    ...
    func rowView(index: Int) -> some View {
        HStack(spacing: 5.2) {
            if index < manager.guesses.count {
                ...
                networkColumn(networks: guess.guessedInfo.Network ?? [], correctNetworks: manager.correctCastInfo?.Network ?? [])
                guessColumn(text: manager.getCoStarDisplayText(for: guess), width: 66, color: manager.getColor(for: guess, category: .shows))
            } else {
    ...
    func networkColumn(networks: [String], correctNetworks: [String]) -> some View {
        ...
        let text = commonNetworks.joined(separator: ", ")
        let color = commonNetworks.count > 1 ? Color("AccentPurple") : Color("AccentGreen")
        return Text(text)
            .font(Font.custom("Poppins-Bold", size: 11))
            .foregroundColor(.white)
            .frame(width: 56, height: 60)
...
struct RealiteaseGameView_Previews: PreviewProvider {
    static var previews: some View {
        RealiteaseGameView(manager: RealiteaseManager(), navigateToRealitease: .constant(false), navigateToGame: .constant(true), gameDate: Date())
    }
}
