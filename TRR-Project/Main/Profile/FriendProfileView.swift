...
struct FriendProfileView: View {
    ...
    private func checkFriendStatus() {
        guard let currentUserId = profileManager.currentUserProfile?.id else { return }

        let db = Firestore.firestore()
        let friendRequestRef = db.collection("users").document(currentUserId).collection("friend-requests").document(userId)
        friendRequestRef.getDocument { (document, error) in
            if let document = document, document.exists {
                self.isFriendRequestSent = true
            } else {
                self.checkFriendCollection()
            }
        }
    }

    private func checkFriendCollection() {
        guard let currentUserId = profileManager.currentUserProfile?.id else { return }

        let db = Firestore.firestore()
        let friendsRef = db.collection("users").document(currentUserId).collection("friends").document(userId)
