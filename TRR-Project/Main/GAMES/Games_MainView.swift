...
struct Games_MainView: View {
    ...
    @State private var navigateToConfessions = false
    ...
    var body: some View {
                    ...
                    VStack(spacing: 10) {
                        ...
                        ScrollView(.horizontal, showsIndicators: false) {
                            HStack(spacing: 10) {
                                Button(action: {
                                    gameDate = Date()
                                    print("Setting gameDate for Confessions: \(String(describing: gameDate))")
                                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                                        navigateToConfessions = true
                                    }
                                }) {
                                    GameCardView(title: "CONFESSIONS", description: "GUESS THE CONFESSIONALS.", date: currentDate, color: Color(red: 0.61, green: 0.60, blue: 0.09))
                                }
                                .fullScreenCover(isPresented: $navigateToConfessions) {
                                    if let gameDate = gameDate {
                                        ConfessionsCoverView(navigateToConfessions: $navigateToConfessions, gameDate: gameDate)
                                    } else {
                                        Text("Error: gameDate is nil")
                                    }
                                }
        ...
        .onChange(of: gameDate, perform: { newGameDate in
            if newGameDate != nil {
        ...
        .onChange(of: navigateToRealationsGame, perform: { newValue in
